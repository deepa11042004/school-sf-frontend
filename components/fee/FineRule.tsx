...
export default function FineRule() {
  const [academicYear, setAcademicYear] = useState("2026-27");
  const [searchTerm, setSearchTerm] = useState("");
  const [entriesPerPage, setEntriesPerPage] = useState("10");
  const [currentPage, setCurrentPage] = useState(1);

  const filteredRules = dummyFineRules.filter(
    (rule) =>
      rule.feeHead.toLowerCase().includes(searchTerm.toLowerCase()) ||
      rule.ruleType.toLowerCase().includes(searchTerm.toLowerCase()),
  );

  const totalPages = Math.ceil(filteredRules.length / parseInt(entriesPerPage));
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            <Link href="/fee/fine-rules/create">
                  ...
                  {filteredRules.length === 0 ? (
                    ...
                    currentRules.map((rule) => (
                      ...
                      >
                        ...
                        <TableCell className="py-3  ">{rule.feeHead}</TableCell>
